# -*- coding: utf-8 -*-
"""
Created on Thu May 16 14:54:57 2019
...
def click_image_left(image,pos,  action, timestamp,offset=5):
    img = cv2.imread(image)
    height, width, channels = img.shape
    pyautogui.moveTo(pos[0], pos[1] + r(height / 2,offset),
                     timestamp)
    pyautogui.click(button=action)

def click_image_right(image,pos,  action, timestamp,offset=5):
    img = cv2.imread(image)
    height, width, channels = img.shape
    pyautogui.moveTo(pos[0] + r(width , offset), pos[1] + r(height / 2,offset),
                     timestamp)
    pyautogui.click(button=action)
'''
...
'''
def imagesearch_loop(image, timesample, precision=0.5):
